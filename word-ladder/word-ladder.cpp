...
std::unordered_set<std::string> load_dictionary(
    ...
    if (!file.is_open()) {
        std::cerr << "Błąd: Nie można otworzyć pliku słownika: "
                  << filename << std::endl;
        return dict;
    }
    std::string word;
...
std::vector<std::string> find_shortest_path(
    ...
    if (dictionary.find(start_word) == dictionary.end()) {
        std::cerr << "Błąd: Słowo startowe \"" << start_word_orig
                  << "\" nie znajduje się w słowniku 4-literowych słów."
                  << std::endl;
        return path;
    }
...
int main() {
    std::unordered_set<std::string> dictionary =
        load_dictionary("slowa.txt");

    if (dictionary.empty()) {
        std::cout
            << "Słownik jest pusty lub nie zawiera słów 4-literowych. "
               "Nie można znaleźć ścieżek."
            << std::endl;
        return 1;
    }
    ...
    std::vector<std::string> shortest_path =
        find_shortest_path(start_word_input, end_word_input, dictionary);
