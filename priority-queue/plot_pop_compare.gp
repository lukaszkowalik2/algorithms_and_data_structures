...
plot \
    input_file_set using 1:2 with linespoints pt 6 ps 0.5 title 'Set-based Pop (O(N) expected)', \
    f_lin(x) with lines lw 2 title sprintf('Fit Set O(N): %.2e*x+%.2e', c, d), \
    input_file_binary using 1:2 with linespoints pt 7 ps 0.5 title 'Binary Heap Pop (O(log N) expected)', \
